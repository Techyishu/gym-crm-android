-- Collect a membership renewal as one transaction. The expected date is a
-- compare-and-set token: once one device settles the renewal and advances the
-- date, every other device still showing the old date is rejected.
...
begin
  ...
  if not found then
    return jsonb_build_object('ok', false, 'error', 'No active membership plan found');
  end if;
