-- Adds partial_count so the Reports "Invoices/Paid/Failed" tile can show a
-- "Partial" bucket too — without it, Paid + Failed didn't add up to
-- total_invoices once partial invoices existed, reading as a data bug.
