...
Future<Uint8List> buildInvoicePdf(Map<String, dynamic> inv) async {
  final regularData = await rootBundle.load('assets/fonts/Manrope_400Regular.ttf');
  ...
  final gymName    = (gym?['name'] as String? ?? 'Gym').toUpperCase();
  final address    = settings['address'] as String?;
  final phone      = settings['phone']   as String?;
        ...
        children: [
            ...
            children: [
              pw.Expanded(
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text(gymName, style: pw.TextStyle(font: bold, fontSize: 18)),
                    if (address != null) ...[
                      pw.SizedBox(height: 3),
                      pw.Text(address, style: pw.TextStyle(font: regular, fontSize: 10, color: PdfColors.grey700)),
                    ],
                    if (phone != null)
                      pw.Text(phone, style: pw.TextStyle(font: regular, fontSize: 10, color: PdfColors.grey700)),
                    if (website != null)
                      pw.Text(website, style: pw.TextStyle(font: regular, fontSize: 10, color: PdfColors.grey700)),
                  ],
                ),
              ),
              pw.Column(
                ...
                children: [
                  pw.Text('INVOICE', style: pw.TextStyle(font: regular, fontSize: 9, color: PdfColors.grey500)),
                  pw.SizedBox(height: 2),
                  pw.Text(invNum, style: pw.TextStyle(font: bold, fontSize: 13)),
          ...
          pw.Row(
            children: [
              ...
              if (paidOn != null) ...[
                pw.SizedBox(width: 28),
                _dateCell(regular, bold, 'PAID ON', paidOn, valueColor: PdfColors.green700),
              ],
            ],
          ),
          ...
          // ── Bill To ───────────────────────────────────────────────────
          pw.Text('BILL TO', style: pw.TextStyle(font: regular, fontSize: 9, color: PdfColors.grey500)),
          pw.SizedBox(height: 6),
          pw.Text(memberName, style: pw.TextStyle(font: bold, fontSize: 14)),
          if (memberEmail.isNotEmpty) ...[
            pw.SizedBox(height: 2),
            pw.Text(memberEmail, style: pw.TextStyle(font: regular, fontSize: 12, color: PdfColors.grey700)),
          ],
          ...
          pw.Row(
            children: [
              pw.Expanded(
                child: pw.Text('DESCRIPTION', style: pw.TextStyle(font: regular, fontSize: 9, color: PdfColors.grey500)),
              ),
              pw.Text('AMOUNT', style: pw.TextStyle(font: regular, fontSize: 9, color: PdfColors.grey500)),
            ],
          ),
          pw.SizedBox(height: 8),
          ...
          pw.Row(
            ...
            children: [
              pw.Expanded(
                child: pw.Text(description, style: pw.TextStyle(font: regular, fontSize: 13)),
              ),
          ...
          if (hasDiscount) ...[
            pw.Padding(
              ...
              child: pw.Row(
                ...
                children: [
                  pw.Text('DISCOUNT', style: pw.TextStyle(font: regular, fontSize: 10, color: PdfColors.green700)),
                  pw.Text('− ${formatCurrency(discountAmount)}', style: pw.TextStyle(font: bold, fontSize: 13, color: PdfColors.green700)),
                ],
              ),
            ),
            pw.Divider(color: PdfColors.grey300),
          ...
          if (notes != null && notes.isNotEmpty) ...[
            pw.SizedBox(height: 14),
            pw.Text('NOTES', style: pw.TextStyle(font: regular, fontSize: 9, color: PdfColors.grey500)),
            pw.SizedBox(height: 6),
            pw.Text(notes, style: pw.TextStyle(font: regular, fontSize: 12, color: PdfColors.grey700)),
          ],
          ...
          pw.Row(
            mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
            children: [
              pw.Text(invNum, style: pw.TextStyle(font: regular, fontSize: 9, color: PdfColors.grey400)),
              pw.Text('Powered by GymCRM', style: pw.TextStyle(font: regular, fontSize: 9, color: PdfColors.grey400)),
            ],
          ),
        ],
      ),
...
String invoiceNumber(String id, String createdAt) {
...
pw.Widget _dateCell(
  ...
  PdfColor valueColor = PdfColors.grey900,
    ...
    pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(label, style: pw.TextStyle(font: regular, fontSize: 9, color: PdfColors.grey500)),
        pw.SizedBox(height: 3),
        pw.Text(value, style: pw.TextStyle(font: bold, fontSize: 12, color: valueColor)),
      ],
    );
