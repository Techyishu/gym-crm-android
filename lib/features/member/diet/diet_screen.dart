...
final _dietPlansProvider = FutureProvider<List<Map<String, dynamic>>>((ref) async {
            ...
            child: ListView(
              ...
              children: [
                _DietHeader(
                  ...
                  planIndex: idx,
                  onSwitchPlan: () => setState(() => _planIndex = (idx + 1) % list.length),
                ),
                Padding(
                  ...
                  child: Column(children: [
                    ...
                    if (meals.isEmpty)
                      ...
                      Container(
                        ...
                        child: Column(
                          children: meals.asMap().entries.map((e) => _MealRow(
                            meal: e.value,
            ...
            if (planCount > 1)
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(color: AppTheme.darkCard2, borderRadius: BorderRadius.circular(10)),
            ...
            children: [
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  const Text('Daily target', style: TextStyle(fontSize: 12.5, color: AppTheme.onDarkSoft)),
                ]),
              ),
              Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                Text('$calories', style: AppTheme.numberStyle(fontSize: 26, color: AppTheme.mintOnDark, height: 1)),
  ...
  Widget build(BuildContext context) {
    ...
    Widget bar(String label, int grams, Color color) {
      return Padding(
        ...
        child: Column(children: [
          Row(children: [
            Text(label, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppTheme.ink)),
            const Spacer(),
            Text('${grams}g', style: AppTheme.numberStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppTheme.inkSoft)),
          ]),
...
class _MealRowState extends State<_MealRow> {
  ...
  Widget build(BuildContext context) {
    ...
    final items = (meal['items'] as List? ?? []).cast<Map<String, dynamic>>();
    final foods = items.map((it) => (it['food'] as String? ?? '').trim())
    ...
    return Container(
      decoration: BoxDecoration(
        border: widget.isLast ? null : const Border(bottom: BorderSide(color: AppTheme.border, width: 0.7)),
      ),
      child: InkWell(
          ...
          child: Column(children: [
            Row(children: [
              Container(
                ...
                child: Text(
                  time.isNotEmpty ? _timeBadge(time) : name.substring(0, 1).toUpperCase(),
                  style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w800, color: badgeFg),
                ),
              ),
              ...
              if (kcal > 0)
                Text('$kcal', style: AppTheme.numberStyle(fontSize: 15, fontWeight: FontWeight.w800)),
            ]),
              ...
              ...items.map((it) {
                ...
                return Padding(
                  ...
                  child: Row(children: [
                    Expanded(
                      child: Text(
                        [it['food'] as String? ?? '—', if (qty.isNotEmpty) qty].join(' · '),
                        style: const TextStyle(fontSize: 12.5, color: AppTheme.ink),
                      ),
                    ),
                    if (c != null)
                      Text('$c kcal', style: const TextStyle(fontSize: 11.5, color: AppTheme.inkHint)),
                  ]),
                );
              }),
            ],
        ...
        children: [
          ...
          Text('No diet plans yet',
              style: TextStyle(
                  ...
                  color: AppTheme.ink)),
          SizedBox(height: 8),
          Text('Your trainer will assign a plan soon',
