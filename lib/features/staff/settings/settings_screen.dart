            ...
            children: [
              ...
              _SettingsCard(
                items: [
                        ...
                        try {
                          ...
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(
                                  'Could not open the ${platform_info.isIOS ? 'App Store' : 'Play Store'}. Please try again.',
                                ),
                              ),
                            );
                          }
                  ...
                  // Lives here since the dashboard header went down to the
          ...
          children: items.asMap().entries.map((e) {
            ...
            return Column(
              children: [
                ...
                if (!isLast) const Divider(height: 1, indent: 50, endIndent: 0),
              ],
            );
          }).toList(),
        ),
...
class _GymDetailsSheetState extends ConsumerState<_GymDetailsSheet> {
  ...
  Future<void> _save(String gymId) async {
    ...
    try {
      ...
      await Supabase.instance.client
          .from('gyms')
          .update({'name': _nameCtrl.text.trim(), 'settings': settings})
          .eq('id', gymId);
                ...
                items: [
                  for (final c in _currencies)
                    DropdownMenuItem(
                      value: c,
                      child: Text(
                        '$c (${NumberFormat.simpleCurrency(name: c).currencySymbol})',
                      ),
                    ),
                ],
...
class _PaymentsSheetState extends ConsumerState<_PaymentsSheet> {
  ...
  Future<void> _disconnect() async {
    ...
    try {
      await Supabase.instance.client.rpc(
        'save_razorpay_keys',
        params: {'p_key_id': null, 'p_key_secret': null},
      );
      if (mounted) {
        Navigator.pop(context);
        ScaffoldMessenger.of(
...
class _SupportTicketSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_SupportTicketSheet> createState() =>
      _SupportTicketSheetState();
}
          ...
          ? Padding(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    'We\'ll get back to you at ${Supabase.instance.client.auth.currentUser?.email ?? 'your email'}.',
                    textAlign: TextAlign.center,
                    style: const TextStyle(
              ...
              return Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      formatDateFromString(t['created_at'] as String?),
                      style: const TextStyle(
                        fontSize: 12,
                        color: AppTheme.inkHint,
                      ),
...
class _RegistrationLinkSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_RegistrationLinkSheet> createState() =>
      _RegistrationLinkSheetState();
}

class _RegistrationLinkSheetState
    extends ConsumerState<_RegistrationLinkSheet> {
  String? _gymId;
  bool _enabled = false;
  String _token = '';
  String? _expiresAt;
  ...
  String _formatExpiry() {
    ...
    if (diff.inHours >= 24)
      return 'Expires in ${diff.inDays}d ${diff.inHours % 24}h';
    if (diff.inHours > 0)
  ...
  String get _link =>
      _token.isEmpty ? '' : '$_registrationBaseUrl/register/$_token';
  ...
  Future<void> _toggle() async {
    ...
    try {
      await Supabase.instance.client
          .from('gyms')
          .update({'registration_enabled': !_enabled})
          .eq('id', _gymId!);
      if (mounted) setState(() => _enabled = !_enabled);
      _toast(
        _enabled ? 'Registration link enabled' : 'Registration link disabled',
      );
    } catch (e) {
            ...
            children: [
              Row(
                children: [
                  const Expanded(
                    child: Text(
                      'Share so new members can sign up themselves',
                      style: TextStyle(fontSize: 13.5, color: AppTheme.inkSoft),
                    ),
                  ),
                  Switch(
...
class BiometricDeviceSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<BiometricDeviceSheet> createState() =>
      BiometricDeviceSheetState();
}
          ...
          : Column(
              ...
              children: [
                ...
                const _SetupStep(
                  number: 1,
                  text:
                      'Connect the device to your router — plug a LAN cable into the device '
                ...
                const _SetupStep(
                  number: 3,
                  text:
                      'On the device, go to Menu → Comm → Ethernet and set: DHCP OFF, '
                ...
                _SetupStep(
                  number: 4,
                  text:
                      'On the device, go to Menu → Comm → Cloud Server and set: '
                ...
                const SizedBox(height: 10),
                const Text(
                  'Multi-biometric (fingerprint + face)',
                  style: TextStyle(
                    fontSize: 11,
                    fontWeight: FontWeight.w700,
                    color: AppTheme.inkSoft,
