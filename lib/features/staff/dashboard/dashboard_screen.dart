...
final _dashboardDataProvider = FutureProvider<Map<String, dynamic>>((ref) async {
  ...
  final results = await Future.wait([
    Future.wait<PostgrestResponse<List<Map<String, dynamic>>>>([
      ...
      client.from('members').select('id').eq('gym_id', gymId).gte('created_at', startOfMonth).count(CountOption.exact),
  ...
  final counts = results[0] as List<PostgrestResponse<List<Map<String, dynamic>>>>;
...
class _DashboardBody extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final ownerName = (profile?['full_name'] as String?) ??
            ...
            child: Row(children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: s.done ? AppTheme.statusActive : AppTheme.surface2,
          ...
          child: GestureDetector(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              Container(
                width: 38, height: 38,
                decoration: BoxDecoration(
                  color: item.accent ? AppTheme.accent : AppTheme.surface2,
              ...
              Padding(
                ...
                child: FittedBox(
                  ...
                  child: Text(item.label,
                    maxLines: 1,
                    style: const TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700, color: AppTheme.inkSoft)),
                ),
              ),
            ]),
          ),
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        Text('$todayCount', style: AppTheme.numberStyle(fontSize: 15, color: AppTheme.accent)),
        const Spacer(),
        GestureDetector(
        ...
        CardList(
          children: checkins.map((ci) {
            ...
            return Padding(
              ...
              child: Row(children: [
                InitialsAvatar(name: label, size: 36, photo: member?['avatar_url'] as String?),
                ...
                Text(_fmtTime(ci['checked_in_at'] as String?),
                  style: const TextStyle(fontSize: 12.5, color: AppTheme.inkSoft,
                    fontFeatures: AppTheme.tabularFigures)),
                const SizedBox(width: 10),
                Container(
                  width: 24, height: 24,
                  decoration: const BoxDecoration(color: AppTheme.statusActiveBg, shape: BoxShape.circle),
                  child: const Icon(Icons.check, size: 14, color: AppTheme.statusActive),
                ),
              ]),
            );
          }).toList(),
                ...
                Expanded(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(label,
                      ...
                      style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppTheme.ink)),
                    if (when.isNotEmpty)
                      Text(when, style: const TextStyle(fontSize: 12, color: AppTheme.inkSoft)),
                  ]),
                ),
                const SizedBox(width: 8),
                Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                  Text(_rupees(amount), style: AppTheme.numberStyle(fontSize: 15, color: AppTheme.statusActive)),
...
class _NewLeads extends StatelessWidget {
  ...
  static StatusPill _pillFor(String status) => switch (status.toLowerCase()) {
    ...
    _           => StatusPill(label: 'New', color: AppTheme.statusDanger, bg: AppTheme.statusDangerBg),
  };
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (leads.isEmpty)
        Container(
          ...
          padding: const EdgeInsets.symmetric(vertical: 20),
          decoration: AppTheme.cardDecoration(),
          child: const Center(child: Text('No enquiries yet',
            style: TextStyle(fontSize: 13, color: AppTheme.inkHint))),
        )
      else
        CardList(
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(dateLabel, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppTheme.inkSoft)),
        ...
        GestureDetector(
          ...
          child: Container(
            width: 38, height: 38,
            decoration: BoxDecoration(color: AppTheme.darkCard, borderRadius: BorderRadius.circular(13)),
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('New members', style: TextStyle(fontSize: 12, color: AppTheme.onDarkSoft)),
                const SizedBox(height: 4),
                Text('$newMembers', style: AppTheme.numberStyle(fontSize: 18, color: AppTheme.onDark)),
              ]),
            ),
          ]),
        ]),
...
class _StatRow extends StatelessWidget {
  ...
  const _StatRow({required this.active, required this.checkins, required this.renewals});

  @override
  Widget build(BuildContext context) {
    return Row(children: [
      Expanded(child: StatTileLight(label: 'Active', value: '$active',
        onTap: () => context.push('/staff/members'))),
      const SizedBox(width: 8),
      Expanded(child: StatTileLight(label: 'Check-ins', value: '$checkins',
        onTap: () => context.push('/staff/check-in'))),
      const SizedBox(width: 8),
      Expanded(child: StatTileLight(label: 'Renewals', value: '$renewals',
...
class _AttentionRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(children: [
        InitialsAvatar(name: name, size: 40, photo: member['avatar_url'] as String?),
        const SizedBox(width: 12),
        Expanded(
          child: GestureDetector(
...
class _CollectPaymentSheetState extends ConsumerState<_CollectPaymentSheet> {
  ...
  @override
  void initState() { super.initState(); _autofill(); }
  ...
  Future<void> _collect() async {
    ...
    if (amountText.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enter an amount'))); return;
    }
    ...
    try {
      ...
      if (mounted) {
        ...
        widget.onPaid();
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Payment collected'), backgroundColor: AppTheme.statusActive));
      }
    } catch (e) {
      debugPrint('[GymCRM] collect error: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to collect payment. Please try again.')));
      ...
      child: SingleChildScrollView(
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text('Collect Payment', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w700, color: AppTheme.ink)),
            IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context)),
          ]),
          const SizedBox(height: 8),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
            decoration: BoxDecoration(color: AppTheme.activeBg, borderRadius: BorderRadius.circular(12)),
            child: Row(children: [
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: selected ? AppTheme.ink : AppTheme.border, width: selected ? 1.5 : 1),
                ),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(m.$3, size: 16, color: selected ? Colors.white : AppTheme.inkSoft),
                  const SizedBox(width: 6),
                  Text(m.$2, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: selected ? Colors.white : AppTheme.ink)),
                ]),
              ),
            );
          }).toList()),
          ...
          TextFormField(
            controller: _refCtrl,
            decoration: InputDecoration(labelText: switch (_method) {
